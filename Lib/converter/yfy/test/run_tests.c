...
bool test_basic_functions() {
    ...
    return (yfy_get_module_voltage(0, &voltage) == YFY_OK && voltage == 48.0f) &&
           (yfy_get_module_group(0, &group) == YFY_OK && group == 1) &&
           (yfy_get_module_voltage(MODULE_NUM, &voltage) == YFY_ERROR_INVALID_ADDR) &&
           (yfy_get_module_voltage(0, NULL) == YFY_ERROR_NULL_POINTER);
}
...
bool test_status_functions() {
    ...
    return (yfy_get_module_limit_power(0, &status) == YFY_OK) &&
           (yfy_get_module_dc_side_fault(0, &status) == YFY_OK) &&
           (yfy_get_output_short_circuit(0, &status) == YFY_OK) &&
           (yfy_get_module_limit_power(MODULE_NUM, &status) == YFY_ERROR_INVALID_ADDR);
}
...
bool test_voltage_current() {
    ...
    return (yfy_get_ac_input_voltage_ab(0, &voltage) == YFY_OK && voltage == 380) &&
           (yfy_get_max_voltage(0, &voltage) == YFY_OK && voltage == 50) &&
...
bool test_group_functions() {
    ...
    return (yfy_get_group_voltage(1, &voltage) == YFY_OK && voltage == 48.0f) &&
           (yfy_get_group_current(1, &current) == YFY_OK && current == 100.0f) &&
           (yfy_get_group_module_num(1, &module_num) == YFY_OK && module_num == 4) &&
           (yfy_get_group_voltage(0, &voltage) == YFY_ERROR_INVALID_GROUP) &&
...
bool test_system_functions() {
    ...
    return (yfy_get_sys_voltage(&voltage) == YFY_OK && voltage == 480.0f) &&
           (yfy_get_sys_current(&current) == YFY_OK && current == 1000.0f) &&
           (yfy_get_sys_module_num(&module_num) == YFY_OK && module_num == 12) &&
           (yfy_get_sys_voltage(NULL) == YFY_ERROR_NULL_POINTER);
}
...
bool test_32bit_macros() {
    ...
    return (data == 0x0A8305F0) &&
           (YFY_EXTRACT_ERROR_CODE(data) == 0x02) &&
           (YFY_EXTRACT_DEVICE_ID(data) == 0x0A) &&
           (YFY_EXTRACT_CMD(data) == 0x03) &&
           (YFY_EXTRACT_DEST_ADDR(data) == 0x05) &&
           (YFY_EXTRACT_SRC_ADDR(data) == 0xF0);
}
...
bool test_boundary_conditions() {
    ...
    // 测试所有状态函数（采样）
    return (yfy_get_module_limit_power(0, &status) == YFY_OK) &&
           (yfy_get_module_id_duplicate(0, &status) == YFY_OK) &&
           (yfy_get_module_severe_unbalanced(0, &status) == YFY_OK) &&
           (yfy_get_three_phase_input_missing(0, &status) == YFY_OK) &&
           (yfy_get_three_phase_input_unbalanced(0, &status) == YFY_OK) &&
           (yfy_get_input_undervoltage(0, &status) == YFY_OK) &&
           (yfy_get_input_overvoltage(0, &status) == YFY_OK) &&
           (yfy_get_module_pfc_fault(0, &status) == YFY_OK);
}
...
bool quick_smoke_test() {
    ...
    // 只测试最关键的几个函数
    return (yfy_get_module_voltage(0, &voltage) == YFY_OK) &&
           (yfy_get_group_voltage(1, &voltage) == YFY_OK) &&
           (yfy_get_sys_voltage(&voltage) == YFY_OK) &&
           (yfy_get_module_limit_power(0, &status) == YFY_OK) &&
           (YFY_SET_ID(0, 1, 2, 3, 4) != 0);
}
