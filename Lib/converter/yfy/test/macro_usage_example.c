...
void demonstrate_bit_layout()
{
    ...
    printf("  [28:26] 错误码: %d%d%d (0x%X)\n", (data_packet >> 28) & 1, (data_packet >> 27) & 1, (data_packet >> 26) & 1, YFY_EXTRACT_ERROR_CODE(data_packet));
    ...
    printf("  [21:16] 命令号: %d%d%d%d%d%d (0x%02X)\n", (data_packet >> 21) & 1, (data_packet >> 20) & 1, (data_packet >> 19) & 1, (data_packet >> 18) & 1,
           (data_packet >> 17) & 1, (data_packet >> 16) & 1, YFY_EXTRACT_CMD(data_packet));
...
void demonstrate_common_commands()
{
    ...
    struct
    {
        ...
        uint8_t dest_addr;
        uint8_t src_addr;
    } commands[] = {{"读取模块0电压", 0, DEVICE_ID, 0x03, 0, MONITOR_ADDR},
    ...
    {
        uint32_t cmd_data = YFY_SET_ID(commands[i].error_code, commands[i].device_id, commands[i].cmd, commands[i].dest_addr, commands[i].src_addr);
...
void demonstrate_data_parsing()
{
    ...
    const char* descriptions[] = {"正常电压读取命令", "正常组数据读取命令", "错误响应数据包"};
