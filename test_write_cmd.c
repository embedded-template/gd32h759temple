...
#define YYLN_SET_ID(protocol, monitor_addr, module_addr, production_day, serial_low) \
    ((uint32_t)(((protocol) & 0x0F) << 25) | \
     (uint32_t)(((monitor_addr) & 0x0F) << 21) | \
     (uint32_t)(((module_addr) & 0x7F) << 14) | \
     (uint32_t)(((production_day) & 0x1F) << 9) | \
     (uint32_t)((serial_low) & 0x1FF))
...
bool test_send(uint32_t id, uint8_t* pdata)
{
    ...
    uint32_t user_data = ((uint32_t)pdata[4] << 24) | 
                        ((uint32_t)pdata[5] << 16) | 
                        ((uint32_t)pdata[6] << 8) | 
                        ((uint32_t)pdata[7]);
...
void yyln_send_write_cmd(uint8_t monitor_addr, uint8_t module_addr, 
                        yyln_message_type_e message_type, yyln_message_cmd_e message_cmd, 
                        uint8_t production_day, uint16_t serial_low, uint8_t* pdata)
{
    // 组装CAN ID
...
void test_write_commands(void)
{
    ...
    // 测试4: 最大32位值 0xFFFFFFFF
    printf("测试4: 设置最大32位值\n");
    uint8_t max_data[4] = {0xFF, 0xFF, 0xFF, 0xFF}; // Big Endian: 4294967295
    yyln_send_write_cmd(0x01, 0x01, yyln_SetData, yyln_VoutReference, 15, 100, max_data);
}
...
void test_data_format_explanation(void)
{
    ...
    printf("常用数值示例:\n");
    printf("  5000 (0x1388) -> [0x00, 0x00, 0x13, 0x88]\n");
    printf("  2000 (0x07D0) -> [0x00, 0x00, 0x07, 0xD0]\n");
    printf("  65535 (0xFFFF) -> [0x00, 0x00, 0xFF, 0xFF]\n");
    printf("  100000 (0x186A0) -> [0x00, 0x01, 0x86, 0xA0]\n");
}
